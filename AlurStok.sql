...
where IdRetailer = @idRetailer
  and Bulan = month(@tglAkhirBefore)
  and tahun = year(@tglAkhirBefore)
  and IdKecamatan = @idKecamatan

select *
from udfMigRdkkUnpivot()


CREATE function [dbo].[udfMigRdkkUnpivot](idRetailer varchar(12), @bulan int, @tahun int, @idKecamatan varchar(10))
    RETURNS @retStokAwal TABLE
                         (
                             idRetailer       varchar(12)  not null,
                             idKecamatan      nvarchar(12) NOT NULL,
                             idProdukRetailer int          NOT NULL,
                             stokAwal         int          NOT NULL
                         )
    --function to resulting stokawal by periodic of month
    --used in sprocCalculateStokv3
as
begin
    if not exists (select *
                   ...
                   where IdRetailer = @idRetailer
                     ...
                     and IdKecamatan = @idKecamatan)
        begin
            insert into @retStokAwal (idRetailer, idKecamatan, idProdukRetailer, stokAwal)
            select KodeRetailer, a.IdKecamatan, b.IdProdukRetailer, sum(Qty) Qty
            from pkp a
                     inner join PkpDetail b on a.Id = b.IdPkp
            where a.KodeRetailer = @idRetailer
              and month(DocDate) = @bulan
              and year(DocDate) = @tahun
              and IdKecamatan = @idKecamatan
              and status = 'r'
              and NoPkp like 'IPKP%'
            group by KodeRetailer, a.IdKecamatan, b.IdProdukRetailer
        end
    else
        begin
            insert into @retStokAwal (idRetailer, idKecamatan, idProdukRetailer, stokAwal)
            select IdRetailer, IdKecamatan, IdProdukRetailer
            from RetailerStok
            where IdRetailer = @idRetailer
              and Bulan = @bulan
              and Tahun = @tahun
...
select * from pkp where NoPkp like 'IPK%' and IdKecamatan like '51%' and year(DocDate) = 2023